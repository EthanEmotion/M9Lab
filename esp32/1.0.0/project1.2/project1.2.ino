...
// TODO:
// 1) remote command per controllare diorama da telecomando + reset + panicButton + printSystemStatus(?)
// 2) scambi per far entrare treni o mandarli
...
void stopAndTrain(int idTrain, bool invert){
	...
	_print("Stop & ");	
	(invert) ? _println("Invert") : _println("Go");
...
void doMainCode(){
   ...
   _println("Execute Maincode");        
   if (checkIfAllTrainIsStopped()){
		delay(beforeStartInterval);
		int randIdTrain = random(1, MY_TRAIN_LEN) - 1;
